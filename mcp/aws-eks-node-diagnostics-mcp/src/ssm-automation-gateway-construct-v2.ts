...
export interface SsmAutomationGatewayV2Props {
   ...
   * Restricted tools perform invasive operations (network packet captures,
   * container namespace entry via nsenter) and are completely absent from the
   * MCP tool surface unless listed here.
   *
   * SECURITY: enabling `tcpdump_capture` does NOT bypass human approval — when
   * `requireCollectionApproval` is on (the default), every capture runs through
   * a wrapper SSM Automation document that pauses at a native `aws:approve`
   * step until a designated approver approves it in the Systems Manager console.
   * @default [] (tcpdump tools are not available)
   */
  readonly enableRestrictedTools?: string[];

  /**
   * Presigned URL expiration in seconds for pcap downloads generated by the
   * `tcpdump_*` tools. Network captures may contain credentials in transit and
   * other sensitive payloads, so they get a much shorter window than ordinary
   * log artifacts. Capped at 300s by the Lambda.
   * @default 60
   */
  readonly pcapPresignedUrlExpirationSeconds?: number;

  /**
   * Size (bytes) above which a completed pcap capture is flagged as oversized
   * in tool responses (advisory warning only).
...
export class SsmAutomationGatewayV2Construct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: SsmAutomationGatewayV2Props = {}) {
    ...
    // Restricted tools (tcpdump) are opt-in; tcpdump_capture is additionally
    // approval-gated at runtime (see the approval section below).
    ...
    // NOTE: The Lambda intentionally has NO ssm:SendCommand permission while
    // the approval workflow is on. Log collection AND tcpdump captures run via
    // ssm:StartAutomationExecution (granted above) using automation documents
    // whose runCommand steps execute under the ssmAutomationRole — not this
    // role — and the tcpdump wrapper pauses at a native aws:approve step first.
    // This means a poisoned agent cannot run shell commands on nodes without a
    // human approving each capture in the SSM console.
    //
    // The ONLY exception: when the operator explicitly disables approval
    // (requireCollectionApproval: false — supervised/test deployments) AND
    // enables the restricted tcpdump tools, the Lambda needs direct
    // SendCommand for the legacy capture path. Even then it is scoped to the
    // allowed documents (AWS-RunShellScript) and EKS-tagged instances only.
    if ((props.requireCollectionApproval ?? true) === false
        && enabledRestrictedTools.includes('tcpdump_capture')) {
      lambdaExecutionRole.addToPolicy(new iam.PolicyStatement({
        sid: 'SSMSendCommandDocsTcpdump',
        effect: iam.Effect.ALLOW,
        actions: ['ssm:SendCommand'],
        resources: ssmDocResources,
      }));
      lambdaExecutionRole.addToPolicy(new iam.PolicyStatement({
        sid: 'SSMSendCommandInstancesTcpdump',
        effect: iam.Effect.ALLOW,
        actions: ['ssm:SendCommand'],
        resources: ssmInstanceResources,
        conditions: {
    ...
    // Wrapper: approve → tcpdump packet capture (single instance, via
    // AWS-RunShellScript). The capture script is built and validated by the
    // MCP Lambda (BPF-filter allowlist, interface/pod-name/PID pattern checks)
    // and passed through the Commands parameter, where the approver can review
    // it verbatim on the execution page before approving. The Lambda has no
    // ssm:SendCommand of its own in approval mode — this wrapper (running as
    // the ssmAutomationRole) is the only way a capture reaches a node, and it
    // never proceeds past the aws:approve step without a human decision.
    const tcpdumpApprovalDocName = `${cdk.Stack.of(this).stackName}-tcpdump-with-approval`;
    new ssm.CfnDocument(this, 'TcpdumpApprovalDocument', {
      ...
      content: {
        schemaVersion: '0.3',
        description:
          'Human-in-the-loop tcpdump packet capture: pauses at a native aws:approve '
          + 'step until a designated approver approves in the Systems Manager console, '
          + 'then sends the capture script to the target EKS node via AWS-RunShellScript.',
        assumeRole: '{{ AutomationAssumeRole }}',
        parameters: {
          InstanceId: {
            type: 'String',
          ...
          Commands: {
            type: 'String',
            description:
              'Capture script built and input-validated by the MCP Lambda. '
              + 'Approvers: review this parameter on the execution page before approving.',
          },
          ExecutionTimeoutSeconds: {
            type: 'String',
            description: 'Run Command execution timeout (capture duration + buffer)',
            allowedPattern: '^\\d{2,4}$',
            default: '240',
          },
          DurationSeconds: {
            type: 'String',
            description: 'Capture duration in seconds (shown to approvers)',
            allowedPattern: '^\\d{1,3}$',
          },
          Interface: {
            type: 'String',
            description: 'Network interface captured (shown to approvers)',
            allowedPattern: '^[a-zA-Z0-9\\-\\.]+$',
          },
          BpfFilter: {
            type: 'String',
            description: 'BPF filter expression, or "none" (shown to approvers)',
            allowedPattern: '^[^\\r\\n]{1,256}$',
          },
          CaptureScope: {
            type: 'String',
            description: 'host, pod/<ns>/<name>, or container/PID-<pid> (shown to approvers)',
            allowedPattern: '^[^\\r\\n]{1,320}$',
          },
          AutomationAssumeRole: {
            type: 'String',
            description: 'Role the automation runs as',
          },
          Approvers: {
            type: 'StringList',
            description: 'IAM principals allowed to approve this capture',
          },
          SNSTopicArn: {
            type: 'String',
            description: 'Topic notified when approval is requested',
        ...
        mainSteps: [
          {
            ...
            inputs: {
              NotificationArn: '{{ SNSTopicArn }}',
              Message:
                'An MCP agent requested a tcpdump PACKET CAPTURE on instance '
                + '{{ InstanceId }} — scope: {{ CaptureScope }}, interface '
                + '{{ Interface }}, duration {{ DurationSeconds }}s, BPF filter: '
                + '{{ BpfFilter }}. Approving runs the capture script (this '
                + 'execution\'s Commands parameter — review it before approving) as '
                + 'root on that node via AWS-RunShellScript and uploads the pcap to '
                + 'S3. Captured packets may contain sensitive payloads. Approve or '
                + 'deny this execution in the Systems Manager console (Automation → '
                + 'Executions) or via ssm send-automation-signal.',
              MinRequiredApprovals: 1,
              Approvers: '{{ Approvers }}',
            },
          },
          {
            ...
            inputs: {
              ...
              Comment: 'tcpdump capture (human-approved) for {{ InstanceId }}',
            },
          },
        ],
      },
    ...
    new cdk.CfnOutput(this, 'CollectionApprovalDocuments', {
      ...
      value: `${collectApprovalDocName}, ${batchApprovalDocName}, ${tcpdumpApprovalDocName}`,
    });
    ...
    this.ssmAutomationFunction = new lambda.Function(this, 'SSMAutomationFunction', {
      ...
      environment: {
        ...
        TCPDUMP_APPROVAL_DOCUMENT: tcpdumpApprovalDocName,
        ...
        TargetConfiguration: {
          Mcp: {
            Lambda: {
              ...
              ToolSchema: {
                InlinePayload: this.getToolSchemaDefinitions(enabledRestrictedTools),
              },
            },
          },
        },
   ...
   * Restricted tools (tcpdump) are only included when explicitly enabled, and
   * tcpdump_capture is approval-gated the same way (M3).
   */
      ...
      tools.push({
        ...
        InputSchema: {
          ...
          Properties: {
            ...
            commandId: {
              Type: 'string',
              Description: 'SSM Command ID from a previous tcpdump_capture call — pass this to poll capture status once it is running',
            },
            executionId: {
              Type: 'string',
              Description: 'SSM Automation execution ID from a pending_approval response — pass this to poll approval status; the capture starts automatically once a human approves',
            },
            region: {
              Type: 'string',
        ...
        OutputSchema: {
          Type: 'object',
          Properties: {
            commandId: { Type: 'string', Description: 'SSM Run Command ID for polling (present once the capture is running)' },
            executionId: { Type: 'string', Description: 'SSM Automation execution ID (approval-gated captures)' },
            approvalConsoleUrl: { Type: 'string', Description: 'SSM console link where an approver approves/denies the capture' },
            humanApproval: { Type: 'object', Description: 'state: pending | approved | denied_or_expired, consoleUrl' },
            instanceId: { Type: 'string' },
